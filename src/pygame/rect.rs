...
impl Rect {
    ...
    pub fn clipline(&self, a:i32,b:i32,c:i32,d:i32) -> Option<((i32,i32),(i32,i32))> {
        let s = (a,b);
        let e = (c,d);
        // 简易线段裁剪实现（AABB）
        let inside_s = self.collidepoint(s);
        let inside_e = self.collidepoint(e);
    ...
    pub fn collidedict<'a, K, V: AsRef<Rect>>(&self, dict: &'a HashMap<K, V>) -> Option<(&'a K, &'a V)> {
    ...
    /// 返回字典中所有碰撞的键值对
    pub fn collidedictall<'a, K, V: AsRef<Rect>>(&self, dict: &'a HashMap<K, V>) -> Vec<(&'a K, &'a V)> {
        dict.iter()
