...
impl RenderResourceAccess {
    ...
    // =========================
    // 查询（Occlusion / Timestamp）
    // =========================

    /// 创建查询集
    ///
    /// 时间戳需要 `Features::TIMESTAMP_QUERY`（`features::core()` 默认已含）。
    /// 结果回读流程：`CommandEncoder::resolve_query_set` →
    /// 回读缓冲 `map_async` → `get_mapped_range`。
    pub fn create_query_set(
        ...
        Arc::new(self.device.create_query_set(&wgpu::QuerySetDescriptor {
            label,
            count,
            ty,
        }))
    ...
    /// 创建查询结果回读缓冲（MAP_READ | COPY_DST | QUERY_RESOLVE，每结果 8 字节）
    pub fn create_query_result_buffer(&self, label: Option<&str>, count: u32) -> wgpu::Buffer {
    ...
    /// 创建裸缓冲（查询结果、staging、uniform 等原始用途；参数开放，语义同 wgpu）
    ...
    /// 向裸缓冲写入数据（`queue.write_buffer` 语义：命令提交前生效）
    ...
    /// 部分更新纹理数据（动态纹理 / 字体图集 / 视频帧上传）
    ///
    /// * `layout`：CPU 侧数据排布（offset / bytes_per_row / rows_per_image）；
    ///   全量覆写一层 mip 时可传默认值（`bytes_per_row` 需 256 字节对齐时由调用方补 padding）
    /// * `origin` / `size`：GPU 侧目标子区域
    ///
    /// ```ignore
    /// access.write_texture(&texture, &rgba_bytes, 0,
    ///     wgpu::Origin3d::ZERO,
